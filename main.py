...
def main(
    ...
    dataset_name = dataset_filepath.split("/")[-1].split(".")[0].rsplit("_", 1)[0]
    ddis_df = pd.read_csv(
        os.path.join(
            "exps_final/comp/bbs/Images",
            f"{dataset_name.replace(dataset_name[:3], dataset_name[:3].upper())}_TM_Results",
            "DDIS/DDIS_iter_512_results.csv",
        )
    )
    diwu_df = pd.read_csv(
        os.path.join(
            "exps_final/comp/",
            "bbs" if "bbs" in dataset_name else dataset_name,
            "Images",
            f"{dataset_name.replace(dataset_name[:3], dataset_name[:3].upper())}_TM_Results",
            "DIWU/DIWU_iter_512_results.csv",
        )
    )
    # ddis_df = pd.read_csv("exps_final/comp/tlpattr/Images/DDIS/DDIS_iter_512_results.csv")
    # diwu_df = pd.read_csv("exps_final/comp/tlpattr/Images/DIWU/DIWU_iter_512_results.csv")
    ...
    for i, row in dataset_csv[start_index:].iterrows():
        ...
        if verbose == True:
            ...
            ddis_y, ddis_x = ddis_df.loc[i][["x", "y"]].astype(int)
            diwu_y, diwu_x = diwu_df.loc[i][["x", "y"]].astype(int)
            ...
            cv2.imwrite(
                ...
                cv2.rectangle(
                    cv2.rectangle(
                        ...
                        (diwu_y, diwu_x),
                        (diwu_y + query_h, diwu_x + query_w),
                        (0, 165, 255),
                        3,
                    ),
                    (ddis_y, ddis_x),
                    (ddis_y + query_h, ddis_x + query_w),
                    (0, 0, 255),
                    3,
                ),
            )
            cv2.imwrite(
                os.path.join(exp_folder, f"{i}_heatmap_ours.png"),
                # cv2.rectangle(
                cv2.rectangle(
                    heatmap1,
                    (query_y, query_x),
            ...
            cv2.imwrite(
                os.path.join(exp_folder, f"{i}_heatmap_DDIS.png"),
                cv2.cvtColor(
                    cv2.rectangle(
                        cv2.cvtColor(
                            cv2.imread(
                                # os.path.join(
                                #     "exps_final/comp/tlpattr/Images/DDIS/seperated",
                                #     f"{i+1}",
                                #     "DDIS Deep_map.jpg",
                                # )
                                os.path.join(
            ...
            cv2.imwrite(
                os.path.join(exp_folder, f"{i}_heatmap_DIWU.png"),
                cv2.cvtColor(
                    cv2.rectangle(
                        cv2.cvtColor(
                            cv2.imread(
                                # os.path.join(
                                #     "exps_final/comp/tlpattr/Images/DIWU/seperated",
                                #     f"{i+1}",
                                #     "DIWU Deep_map.jpg",
                                # )
                                os.path.join(
...
if __name__ == "__main__":
    ...
    exp_folder = "exps_final/bbs100/images"
    # exp_folder = "exps_final/tiny_tlp"
    # exp_folder = "exps_final/tlpattr/images"
                        ...
                        for n_haar_feature in n_haar_features:
                            ...
                            df.to_csv(
                                os.path.join(
                                    ...
                                    f"{dataset_name}_all_time_results_k3s2haar.csv"
