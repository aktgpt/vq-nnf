        ...
        for kernel_size in kernel_sizes:
            try:
                result_csv = pd.read_csv(
                    os.path.join(
                        folder,
                        f"tiny_tlp_dataset_resnet18_n_features_{n_features}_n_cluster_128_n_haar_feature_{n_haar_feature}_scale_{scale}_iou_sr.csv",
                    )
                )
            except:
                continue
...
intervals = np.arange(result_csvs["temp_size"].min(), result_csvs["temp_size"].max(), 200)
result_csvs["Temp Size"] = (
...
sns.lineplot(
    ...
    hue="Method",
    data=result_csvs,
    palette=sns.color_palette(cc.glasbey, len(result_csvs["Method"].unique())),
    errorbar=None,
)
plt.xscale("log")
...
ddis_df = pd.read_csv(
    f"exps/tiny_tlp/gauss/DDIS_iter_results.csv"
    if n_features == 27
    else f"exps/tiny_tlp/gauss/DDIS_iter_deep_results.csv"
)
ddis_df["Method"] = "DDIS"
...
diwu_df = pd.read_csv(
    "exps/tiny_tlp/gauss/DIWU_iter_results.csv"
    if n_features == 27
    else "exps/tiny_tlp/gauss/DIWU_iter_deep_results.csv"
)
diwu_df["Method"] = "DIWU"
