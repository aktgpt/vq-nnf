...
class HaarFiltersFFT(nn.Module):
    def __init__(
        self,
        n_channels,
        kernel_size,
    ...
    def get_query_map(self, x, weights=None):
        ...
        for i, (filter, kernel_size) in enumerate(zip(self.filters, self.kernel_sizes)):
            ...
            # distance_save = (distance - distance.min()) / (distance.max() - distance.min())
            # distance_save = np.pad(
            #     distance_save.cpu().numpy(),
            #     (
            #         (
            #             inp_shape[0] // 2 - distance_save.shape[0] // 2,
            #             inp_shape[0] // 2 - distance_save.shape[0] // 2,
            #         ),
            #         (
            #             inp_shape[1] // 2 - distance_save.shape[1] // 2,
            #             inp_shape[1] // 2 - distance_save.shape[1] // 2,
            #         ),
            #     ),
            #     "constant",
            ...
            # )
            # cv2.imwrite(
            #     f"exps_final/figures/filters_vis/input_fig_{self.filter_names[i]}.png",
            #     cv2.applyColorMap((distance_save * 255).astype(np.uint8), cv2.COLORMAP_JET),
            # )

        # plt.figure()
        # plt.imshow(distances.cpu().numpy())
        # plt.show()
        # distances_save = (distances - distances.min()) / (distances.max() - distances.min())
        # distances_save = np.pad(
        #     distances_save.cpu().numpy(),
        #     (
        #         (
        #             inp_shape[0] // 2 - distances_save.shape[0] // 2,
        #             inp_shape[0] // 2 - distances_save.shape[0] // 2,
        #         ),
        #         (
        #             inp_shape[1] // 2 - distances_save.shape[1] // 2,
        #             inp_shape[1] // 2 - distances_save.shape[1] // 2,
        #         ),
        #     ),
        #     "constant",
        ...
        # )
        # cv2.imwrite(
        #     f"exps_final/figures/filters_vis/final.png",
        #     cv2.applyColorMap((distances_save * 255).astype(np.uint8), cv2.COLORMAP_JET),
        # )
...
class HaarTemplateMatcher:
    def __init__(
        ...
        # U, S, V = torch.pca_lowrank(template_flatten, q=9)
        # self.v = V
        # template_flatten = template_flatten @ V

        self.n_clusters = (
            n_clusters if template_flatten.shape[0] > n_clusters else template_flatten.shape[0]
        )

        # if template_flatten.shape[0] < self.n_clusters:
        # self.n_clusters = template_flatten.shape[0]
        ...
        self.pool1d = nn.MaxPool1d(kernel_size=self.n_clusters, return_indices=True)
        self.unpool1d = nn.MaxUnpool1d(kernel_size=self.n_clusters)
        ...
        one_hot = (
            torch.nn.functional.one_hot(
                choice_cluster.reshape(self.t_w, self.t_h), num_classes=self.n_clusters
            )
            .permute(2, 0, 1)
            .unsqueeze(0)
        ).float()
        self.temp_one_hot = one_hot
        # fig, ax = plt.subplots(8, self.n_clusters // 8, figsize=(20, 20))
        # for i in range(1,self.n_clusters):
        #     ax[i // 8, i % (self.n_clusters // 8)].imshow(
        #         label2rgb(one_hot[0, i].squeeze(0).cpu().numpy(), colors=self.colors, bg_label=0)
        #     )
        ...
        temp_weights = torch.ones(self.n_clusters, device=self.device) / self.n_clusters
        ...
        self.rot_inv_conv = HaarFiltersFFT(
            self.n_clusters,
    ...
    def get_heatmap(self, x):
        ...
        with torch.no_grad():
            ...
            temp_fft = torch.fft.fft2(one_hots, s=one_hots.shape[1:], norm="forward")
            query_fft = torch.fft.fft2(self.temp_one_hot, s=one_hots.shape[1:], norm="forward")
            deform_diff = (
                torch.fft.ifft2((temp_fft * query_fft.conj()), norm="backward").real.squeeze(0)
                # .mean(dim=0)
            )
            # get multiplication along the 1st dimension
            deform_diff = deform_diff.sum(dim=0)
