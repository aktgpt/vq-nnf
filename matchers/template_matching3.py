...
gabor_filter_bank = []
for sigma in (1, 2, 3):
    kernel = np.real(gabor_kernel(1.0, theta=0, sigma_x=sigma, sigma_y=sigma, n_stds=3))
    filter = {"kernel": torch.tensor(kernel).float(), "weight": 1}
    gabor_filter_bank.append(filter)
    for theta in range(1, 3):
        # theta = theta / 4.0 * np.pi
        theta = (2 * theta - 1) * (np.pi / 4)
        kernel = np.real(gabor_kernel(1.0, theta=theta, sigma_x=sigma, sigma_y=sigma, n_stds=3))
        filter = {"kernel": torch.tensor(kernel).float(), "weight": 0.25}
        gabor_filter_bank.append(filter)
...
def get_gaussian_box_filter(kernel_size, sigma, order=(0, 0)):
    ...
    if kernel_size % 2 == 0:
        kernel[center - 1 : center + 1, center - 1 : center + 1] = 1 / 4
    else:
...
def convert_box_to_haar(box_filter, haar_filter_dict):
    haar_weight = haar_filter_dict["weights"]
    repeat_x = box_filter.shape[0] // haar_weight.shape[0]
    repeat_y = box_filter.shape[1] // haar_weight.shape[1]
    haar_filter = haar_weight.repeat_interleave(repeat_x, axis=0).repeat_interleave(
        repeat_y, axis=1
    )

    haar_filter = haar_filter * box_filter
    # haar_filter = ndimage.correlate(box_filter, haar_filter)
    print(haar_filter)
...
class HaarFilters(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.filter_kernel_size = 4
        self.sigmas = [(3, 3), (2, 1), (1, 2)]
    ...
    def get_filters(self, n_channels, kernel_size, scale, scale_weight):
        ...
        for i in range(self.n_haar_features):
            int_filter_weights = convert_box_to_integral(gabor_filter_bank[i]["kernel"])
            filter_weights = gabor_filter_bank[i]["weight"]

            kernel_size = int_filter_weights.shape[:2]
            dilation = ((w - 1) // kernel_size[0], (h - 1) // kernel_size[1])
            ...
            self.filter_weights.append(scale_weight * filter_weights)

    def get_filters2(self, n_channels, kernel_size, scale, scale_weight):
        w = int(kernel_size[0] * scale)
        h = int(kernel_size[1] * scale)

        dilation = (w // self.filter_kernel_size, h // self.filter_kernel_size)
        kernel_size = (self.filter_kernel_size, self.filter_kernel_size)
        # int_filter_weights = convert_box_to_integral(
        #     get_gaussian_box_filter(self.filter_kernel_size, 2, order=self.orders[i])
        # )
        int_filter_weights = convert_box_to_integral(
            get_gaussian_box_filter(self.filter_kernel_size, self.n_haar_features)
        )
        filter = nn.Conv2d(
            n_channels,
            n_channels,
        ...
        filter.weight.data = int_filter_weights[None, None].repeat(n_channels, 1, 1, 1) / (w * h)
        self.kernel_sizes.append((w, h))
        self.filters.append(filter.to(self.device))
        self.filter_weights.append(scale_weight)

    def get_filters3(self, n_channels, kernel_size, scale, scale_weight):
        w = int(kernel_size[0] * scale)
        h = int(kernel_size[1] * scale)
        dilation = ((w - 1) // self.filter_kernel_size, (h - 1) // self.filter_kernel_size)
        kernel_size = (self.filter_kernel_size + 1, self.filter_kernel_size + 1)
        ...
        for filter_name, filter_dict in islice(haar_weight_dict.items(), 0, self.n_haar_features):
            ...
            #     get_gaussian_box_filter(self.filter_kernel_size, 2, order=self.orders[i])
            # )
            int_filter_weights, filter_weight = convert_box_to_integral(
                get_gaussian_box_filter(self.filter_kernel_size, self.sigmas[i])
            )
