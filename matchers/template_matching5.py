...
class HaarTemplateMatcher:
    def __init__(
        ...
        if self.pca_dims is not None:
            U, S, V = torch.pca_lowrank(template_flatten, q=self.pca_dims)
            self.v = V
        ...
        if self.verbose:
            ...
            # cv2.imwrite(
            #     f"exps_final/figures/filters_vis/integral/labels.png", self.template_labels * 255
            # )
        else:
        ...
        # for i in range(cumsum_onehot.shape[1]):
        #     x = cumsum_onehot[0, i].cpu().numpy()
        #     x = ((x) / (x.max().item()) * 255).astype(np.uint8)
        #     cv2.imwrite(
        #         f"exps_final/figures/filters_vis/integral/cumsum_{i+1}.png",
        #         cv2.applyColorMap(x, cv2.COLORMAP_JET),
        #     )
        #     x = (one_hot[0, i].cpu().numpy() * 255).astype(np.uint8)
        #     cv2.imwrite(
        #         f"exps_final/figures/filters_vis/integral/onehot_{i+1}.png",
        #         cv2.applyColorMap(x, cv2.COLORMAP_JET),
        #     )
