...
def main(
    ...
    for i, row in dataset_csv[start_index:].iterrows():
        ...
        template_image = cv2.resize(template_image, (0, 0), fx=exp_scale, fy=exp_scale)

        query_image = cv2.cvtColor(cv2.imread(row["query_path"]), cv2.COLOR_BGR2RGB)
        query_image = cv2.resize(query_image, (0, 0), fx=exp_scale, fy=exp_scale)

        template_bbox = (
            row[["template_x", "template_y", "template_w", "template_h"]].values * exp_scale
        ).astype(int)
        query_bbox = (row[["query_x", "query_y", "query_w", "query_h"]].values * exp_scale).astype(
            int
        )
...
if __name__ == "__main__":
    ...
    # "resnet50", "resnet34",
    n_features = [512]  #  [40, 80, 192, 27]
    n_clusters = [128]  # [4, 8, 16, 32, 64, 128]  # [32, 64, 128] [4, 8, 16, 32, 64]
    n_haar_features = [1, 2, 3, 23]  # [1, 3, 5]
    scales = [2]  # [1, 2, 3, 4]
    exp_scales = [0.25, 0.33, 0.5, 0.66, 0.75, 1]

    # exp_folder = "exps/bbs25"
    # exp_folder = "exps/tiny_tlp"
    exp_folder = "exps_final/tlpattr/scale"
                    ...
                    for scale in scales:
                        for n_haar_feature in n_haar_features:
                            for exp_scale in exp_scales:
                                exp_name = f"{dataset_name}_{model}_n_features_{n_feature}_n_cluster_{n_cluster}_n_haar_feature_{n_haar_feature}_scale_{scale}_scale_{exp_scale}"
                                ...
                                all_exp_scales.append(exp_scale)
                                ...
                                df.to_csv(
                                    os.path.join(
                                        exp_folder,
                                        f"{dataset_name}_all_time_results_k3s2haar_scale_{n_feature}.csv"
