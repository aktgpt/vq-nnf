    ...
    for i in range(0, len(sampled_df), n_samples):
        ...
        for _, row in rows.iterrows():
            ...
            template_matcher = HaarTemplateMatcher(
                template_features, patch_size, n_haar_features=n_haar_features,
            )
        ...
        auc = np.trapz(pr_df1["recall_bins"], pr_df1["mean_precision"])
        auc_desc.set_description(f"ImageID: {query_image_id} AUC: {auc:.3f}")

        recall_bins_repeat = np.repeat(pr_df["recall_bins"].values, i + 1)
        ...
        #         [pd.cut(all_predictions_df["dist_percentile"], np.linspace(0, 1, 500))],
        #     )[["precision", "recall"]]
        #     .mean()
        #     .reset_index()
        # )
        # print(all_predictions_df.shape)
        # pr_dfs.append(all_predictions_df)

        # pr_df_concat = pd.concat(pr_dfs).reset_index(drop=True)
        # pr_df_concat = pr_df_concat.sort_values("recall1", ascending=False).reset_index(drop=True)

        # pr_df_mean = pr_df_concat.groupby("recall1")[["precision", "recall"]].mean().reset_index()
        # pr_df_std = pr_df_concat.groupby("recall1")[["precision", "recall"]].std().reset_index()
        # pr_df_mean.to_csv(f"{exp_folder}_pr_mean.csv", index=False)
        # pr_df_std.to_csv(f"{exp_folder}_pr_std.csv", index=False)

        # replace the values with the midpoints of the intervals
        # pr_df_concat["recall1"] = intervals.apply(lambda x: x.mid)
        # pr_df_concat = pr_df_concat.sort_values("recall1", ascending=False).reset_index(drop=True)

        # pr_df_concat.to_csv(f"{exp_folder}_pr.csv", index=False)
        # pr_df_concat = pr_df_concat.dropna()

        # pr_curve_df = (
        #     pr_df_concat.groupby([pd.cut(pr_df_concat["recall"], np.linspace(0, 1, 100))])[
        #         "precision"
        #     ]
        #     .apply(np.array)
        #     .reset_index()
        # )
        # pr_curve_df["recall"] = pr_curve_df["recall"].apply(lambda x: x.mid)
