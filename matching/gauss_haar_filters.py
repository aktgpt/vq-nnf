...
from .utils import get_center_crop_coords, convert_box_to_integral
...
def get_gauss_haar_integral(kernel_size, sigma, haar_filter_dict):
    ...
    haar_filter = haar_weight.repeat_interleave(repeat_x, axis=0).repeat_interleave(repeat_y, axis=1)
...
class GaussHaarFilters(nn.Module):
    def __init__(
        ...
        self.channel_weights = torch.from_numpy(np.array(channel_weights)).float().to(self.device)
        self.template_shape = template_shape

        rects_to_consider = decimal.Decimal(filters).as_tuple().digits
        filters_to_consider = ["haar_1x"]
        filters_to_consider.extend(
            [k for k in HAAR_CONF_DICT.keys() if int(list(filter(str.isdigit, k))[0]) in rects_to_consider]
        )

        self.n_filters = len(filters_to_consider) * n_scales

        self.filter_kernel_size = (kernel_size, kernel_size) if isinstance(kernel_size, int) else kernel_size
        ...
        assert n_scales == len(scale_weights), "Number of scales and scale weights should be the same"
        ...
        for scale_to_consider, scale_weight in zip(scales_to_consider, scale_weights):
            self.get_filters(n_channels, template_shape, filters_to_consider, scale_to_consider, scale_weight)
    ...
    def get_filters(
        ...
        for filter_name in filters_to_consider:
            ...
            integral_filter_kernel = get_gauss_haar_integral(self.filter_kernel_size, self.sigmas, filter_config)
            ...
            filter.weight.data = integral_filter_kernel[None, None].repeat(n_channels, 1, 1, 1) / (
    ...
    def forward_filter(
        ...
        with torch.no_grad():
            ...
            if out_shape is not None:
                x1, y1, x2, y2 = get_center_crop_coords(y_pad.shape[2], y_pad.shape[3], out_shape[0], out_shape[1])
                y = y_pad[:, :, x1:x2, y1:y2]
            else:
                y = y_pad
            return y
    ...
    def get_query_map(self, x: torch.Tensor, channel_weights: Optional[np.array] = None):
        channel_weights = self.channel_weights if channel_weights is None else channel_weights

        distrib_sim = torch.zeros(x.shape[2:], device=self.device)  # np.zeros(x.shape[2:])
        all_filt_sims = []

        for i, (filter, kernel_size) in enumerate(zip(self.filters, self.scale_kernel_sizes)):
            y = self.forward_filter(x, filter, kernel_size, out_shape=None)

            filter_shift = torch.abs(y - self.template_features[i]) * channel_weights[None, :, None, None]
