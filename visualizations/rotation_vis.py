...
# pgf_with_latex = {"text.usetex": True, "pgf.rcfonts": False, "font.family": "Arial"}
# plt.rcParams.update(pgf_with_latex)


datasets = ["BBS25", "BBS50", "BBS100", "TinyTLP"]
methods_map = {
    1: "$\mathcal{F}_{gauss}$",
...
for dataset in datasets:
    if "BBS" in dataset:
        ddis_df = pd.read_csv("c:/Users/gupta/Desktop/BBS_data/BBS_rot_DDIS_results.csv")
        ddis_df_rot0 = pd.read_csv("c:/Users/gupta/Desktop/BBS_data/BBS_DDIS_results.csv")
        ddis_df_rot0["rotation"] = 0
        ddis_df = pd.concat([ddis_df_rot0, ddis_df]).reset_index(drop=True)
        ddis_df["dataset"] = "BBS" + ddis_df["exp_cat"].astype(str)
        ddis_df["Method"] = "DDIS"

        diwu_df = pd.read_csv("c:/Users/gupta/Desktop/BBS_data/BBS_rot_DIWU_results.csv")
        diwu_df_rot0 = pd.read_csv("c:/Users/gupta/Desktop/BBS_data/BBS_DIWU_results.csv")
        diwu_df_rot0["rotation"] = 0
        diwu_df = pd.concat([diwu_df_rot0, diwu_df]).reset_index(drop=True)
        diwu_df["dataset"] = "BBS" + diwu_df["exp_cat"].astype(str)
        diwu_df["Method"] = "DIWU"

        vqnnf_df = pd.read_csv("BBS_vq_nnf_rotate_results.csv")
    else:
        ddis_df = pd.read_csv("c:/Users/gupta/Desktop/TinyTLP_comp/TinyTLP_27_DDIS_results.csv")
        ddis_df["dataset"] = "TinyTLP"
        ddis_df["Method"] = "DDIS"

        diwu_df = pd.read_csv("c:/Users/gupta/Desktop/TinyTLP_comp/TinyTLP_27_DIWU_results.csv")
        diwu_df["dataset"] = "TinyTLP"
        diwu_df["Method"] = "DIWU"

        vqnnf_df = pd.read_csv("TinyTLP_vq_nnf_rotate_results.csv")
    ...
    vqnnf_df["Rotation"] = vqnnf_df["dataset"].apply(lambda x: x.split("_rot")[-1] if "_rot" in x else 0)
    vqnnf_df["dataset"] = vqnnf_df["dataset"].apply(lambda x: x.split("_rot")[0].split("_")[0])
    scale_to_consider = 3 if "BBS" in dataset else 4
    vqnnf_df["Method"] = f"$S={scale_to_consider}, $" + vqnnf_df["rect_haar_filters"].map(methods_map)

    dataset_df = pd.concat(
        [
            ddis_df.loc[ddis_df["dataset"] == dataset],
            diwu_df.loc[diwu_df["dataset"] == dataset],
            vqnnf_df.loc[vqnnf_df["dataset"] == dataset],
        ]
    ...
    for h in handles:
        h.set_linewidth(5)
    lgd = ax.legend(handles=handles, labels=labels)#, loc="upper left", bbox_to_anchor=(1.05, 1), borderaxespad=0.0)

    plt.savefig(f"figures/rotate/rot_comp_{dataset}.png", dpi=500, bbox_inches="tight")  # , bbox_extra_artists=(lgd,))
    plt.close()
    x = 1
...
#         [
#             pd.read_csv(x)
#             for x in glob.glob(f"{folder}/{dataset}/rot/{dataset}*_dataset_all_time_results_k3s2haar_rotation.csv")
#         ]
#     ).reset_index(drop=True)
#     result_csv["Dataset"] = dataset.upper() if "tiny" not in dataset else "TinyTLP"
#     result_csv["n_haar_features"] = result_csv["n_haar_features"].apply(lambda x: "No Haar" if x == 1 else x)
...
#     )
#     result_csv["Method"] = "S=" + result_csv["scales"].astype(str) + "," + result_csv["n_haar_features"].astype(str)
